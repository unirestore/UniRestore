"""
Adapted from: https://github.com/bethgelab/imagecorruptions
Changes:
- #94 return np.array(image_corrupted, dtype=np.uint8, copy=True) to avoid non-writable tensors
"""
...
def corrupt(image, severity=1, corruption_name=None, corruption_number=-1):
    ...
    if not (corruption_name is None):
        image_corrupted = corruption_dict[corruption_name](
            Image.fromarray(image), severity
        )
    elif corruption_number != -1:
        image_corrupted = corruption_tuple[corruption_number](
            Image.fromarray(image), severity
        )
    else:
        raise ValueError("Either corruption_name or corruption_number must be passed")

    return np.array(image_corrupted, dtype=np.uint8, copy=True)

def init_corruption_function(subset="common"):
    if subset == "common":
        crp_set = [f.__name__ for f in corruption_tuple[:15]]
    elif subset == "clean":
        crp_set = []
    elif subset == "validation":
        crp_set = [f.__name__ for f in corruption_tuple[15:]]
    elif subset == "all":
        crp_set = [f.__name__ for f in corruption_tuple]
    elif subset == "noise":
        crp_set = [f.__name__ for f in corruption_tuple[0:3]]
    elif subset == "blur":
        crp_set = [f.__name__ for f in corruption_tuple[3:7]]
    elif subset == "weather":
        crp_set = [f.__name__ for f in corruption_tuple[7:11]]
    elif subset == "digital":
        crp_set = [f.__name__ for f in corruption_tuple[11:15]]
    else:
        raise ValueError("subset must be one of ['clean', 'common', 'validation', 'all']")
    return crp_set + ["clean"]
