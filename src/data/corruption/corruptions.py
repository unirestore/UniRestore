...
Changes:
- improve glass blur with vectorized numpy instead of numba
- fix frost images path
"""
...
# @njit()
# replace namba.njit with vectorized numpy
def _shuffle_pixels_njit_glass_blur(d0, d1, x, c):
    ...
    for i in range(c[2]):
        ...
        h = np.arange(c[1], d0 - c[1])
        w = np.arange(c[1], d1 - c[1])
        w, h = np.meshgrid(w, h, indexing="xy")

        dx = np.random.randint(-c[1], c[1], size=(d0 - 2 * c[1], d1 - 2 * c[1]))
        dy = np.random.randint(-c[1], c[1], size=(d0 - 2 * c[1], d1 - 2 * c[1]))
        h_prime = h + dy
        w_prime = w + dx
        # Swap pixels
        x_swapped = np.copy(x)
        x_swapped[h, w] = x[h_prime, w_prime]
        x = x_swapped
...
def glass_blur(x, severity=1):
    ...
    return np.clip(gaussian(x / 255.0, sigma=c[0]), 0, 1) * 255
...
def fog(x, severity=1):
    ...
    if len(x_shape) < 3 or x_shape[2] < 3:
        x += (
            c[0]
            * plasma_fractal(mapsize=map_size, wibbledecay=c[1])[: shape[0], : shape[1]]
        )
    else:
        x += (
            c[0]
...
def frost(x, severity=1):
    ...
    filename = [
        "./frost_img/frost1.png",
        "./frost_img/frost2.png",
        "./frost_img/frost3.png",
        "./frost_img/frost4.jpg",
        "./frost_img/frost5.jpg",
        "./frost_img/frost6.jpg",
    ][idx]
...
def snow(x, severity=1):
    ...
    if len(x.shape) < 3 or x.shape[2] < 3:
        ...
        x = c[6] * x + (1 - c[6]) * np.maximum(
            x,
            cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(x.shape[0], x.shape[1], 1) * 1.5
            + 0.5,
        )
    try:
