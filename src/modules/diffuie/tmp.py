...
class Attention(nn.Module):
    ...
    def forward(self, x, task_prompts):
        ...
        raw_spa_attn = (q @ k.transpose(-2, -1))  # spatial attention map: (B, heads, N, N)
        attn = raw_spa_attn * self.scale     
        attn = attn.softmax(dim=-1)               # weighted attention map: (B, heads, N, N) q->k 
        ...
        chan_x = ori_x[:, -self.pixel_no:, :]  # pickup all patch tokens (B, N, C)

        chan_prompts = self.token_trans(ori_task_prompts) # (B, T, C) -> (B, T, N)
        ...
        q = chan_prompts            # (B, T, N)
        k = chan_x.permute(0,2,1)   # (B, C, N)
        v = k                       # (B, C, N)

        # multi-head attention (throught the spatial window slide reshape)
        nh = nw = int(np.sqrt(self.chan_nheads))    
        win_h = self.resolution[0] // nh
        win_w = self.resolution[1] // nw
        q = rearrange(q, 'b t (nh h nw w) -> b (nh nw) t (h w)', nh=nh, nw=nw, h=win_h, w=win_w) # (B, hd, T, wh*ww)
        k = rearrange(k, 'b t (nh h nw w) -> b (nh nw) t (h w)', nh=nh, nw=nw, h=win_h, w=win_w) # (B, hd, C, wh*ww)
        v = rearrange(v, 'b t (nh h nw w) -> b (nh nw) t (h w)', nh=nh, nw=nw, h=win_h, w=win_w) # (B, hd, C, wh*ww)

        raw_chan_attn = (q @ k.transpose(-2, -1))   # (B, hd, T, C)
        attn = raw_chan_attn * self.chan_scale      # (B, hd, T, C)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        chan_x = (attn @ v)                         # (B, hd, T, wh*ww)
        chan_x = rearrange(chan_x, 'b (nh nw) t (h w) -> b t (nh h nw w)', nh=nh, nw=nw, h=win_h, w=win_w)  # (B, T, N)
        raw_chan_attn = rearrange(raw_chan_attn, 'b (nh nw) t c -> b t c nh nw', nh=nh, nw=nw)  # (B, T, C, nh, nw)
        raw_chan_attn = raw_chan_attn, rearrange(attn, 'b (nh nw) t c -> b t c nh nw', nh=nh, nw=nw) # (B, T, C, nh, nw)
...
class Block(nn.Module):
    ...
    def __init__(self, chan_nheads, resolution, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()  # maybe one kind of dropout
...
class TaskPrompter(nn.Module):
    ...
    def forward(self, x):
        ...
        # put prompts in it
        task_prompts1 = self.task_prompts[None].expand(x.shape[0], -1, -1)  # (B, T, d)
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx + 1 in self.select_list: 
                ...
                _cur_task_fea, info = self.cal_task_feature(x, attn_weight, il, info) # patch_emb, attention_map, i-th layer, info?
    ...
    def cal_task_feature(self, x, attn_weight, il, info):
        ...
        '''
        x = rearrange(x, 'b (h w) c -> b c h w', h=self.resolution[0], w=self.resolution[1]) # embed -> feature map
        task_fea = {}
        ...
        for t_idx, task in enumerate(self.p.TASKS.NAMES):
            ...
            cur_attn_weight = spa_attn[:, :, t_idx*self.prompt_len:(t_idx+1)*self.prompt_len, :]  # select the current task (q) to all (k: task+patch)
            cur_attn_weight = cur_attn_weight[:, :, :, self.prompts_len:]  # filter the current task (q) to patch (k)
